...
function finity.new(isdark, gprojectName, thinProject)
	...
	self2.ChangeToggleKey = function(NewKey)
		...
		finityData.UpConnection = finity.gs["UserInputService"].InputEnded:Connect(function(Input)
			if Input.KeyCode == finityData.ToggleKey and not typing then
                ...
                if toggled then
					pcall(self2.container.TweenPosition, self2.container, savedposition, "Sine", "Sine", 0.5, true)
                else
                    savedposition = self2.container.Position;
					pcall(self2.container.TweenPosition, self2.container, UDim2.new(savedposition.Width.Scale, savedposition.Width.Offset, 1.5, 0), "Sine", "Sine", 0.5, true)
				end
			end
		end)
	end
	...
	finityData.UpConnection = finity.gs["UserInputService"].InputEnded:Connect(function(Input)
		if Input.KeyCode == finityData.ToggleKey and not typing then
			...
			if toggled then
				self2.container:TweenPosition(UDim2.new(0.5, 0, 0.5, 0), "Sine", "Sine", 0.5, true)
			else
				self2.container:TweenPosition(UDim2.new(0.5, 0, 1.5, 0), "Sine", "Sine", 0.5, true)
			end
		end
	end)
